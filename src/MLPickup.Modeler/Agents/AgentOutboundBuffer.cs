...
namespace MLModeling.Modeler.Agents
{
    ...
    {
        static readonly IInternalLogger Logger = InternalLoggerFactory.GetInstance<AgentOutboundBuffer>();
        ...
        internal AgentOutboundBuffer(IAgent Agent)
        {
            this.Agent = Agent;
        }

        /// <summary>
        /// Adds the given message to this <see cref="AgentOutboundBuffer"/>. The given
        ...
        /// <summary>
        /// Add a flush to this <see cref="AgentOutboundBuffer"/>. This means all previous added messages are marked
        ...
        void IncrementPendingOutboundBytes(long size, bool invokeLater)
        {
            ...
            if (newWriteBufferSize >= this.Agent.Configuration.WriteBufferHighWaterMark)
            {
        ...
        void DecrementPendingOutboundBytes(long size, bool invokeLater, bool notifyWritability)
        {
            ...
            if (notifyWritability && (newWriteBufferSize == 0
                || newWriteBufferSize <= this.Agent.Configuration.WriteBufferLowWaterMark))
            {
        ...
        public void Progress(long amount)
        {
            ...
            //var p = e.promise;
            //if (p is AgentProgressivePromise)
            //{
            //    long progress = e.progress + amount;
            //    e.progress = progress;
            //    ((AgentProgressivePromise)p).tryProgress(progress, e.Total);
            //}
        }
        ...
        /// <see cref="AbstractAgent.DoWrite(AgentOutboundBuffer)"/>. Refer to
        /// <see cref="TcpSocketAgent.DoWrite(AgentOutboundBuffer)"/> for an example.
        /// </para>
        /// </summary>
        ...
        /// did not exceed the write watermark of the <see cref="IAgent"/> and no user-defined writability flag
        ...
        void SetUserDefinedWritability(int index)
        {
            ...
            while (true)
            {
                ...
                if (Interlocked.CompareExchange(ref this.unwritable, newValue, oldValue) == oldValue)
                {
                    ...
                    {
                        this.FireAgentWritabilityChanged(true);
                    }
                    break;
                }
            }
        ...
        void SetWritable(bool invokeLater)
        {
            while (true)
            {
                ...
                if (Interlocked.CompareExchange(ref this.unwritable, newValue, oldValue) == oldValue)
                {
                    if (oldValue != 0 && newValue == 0)
                    {
                        this.FireAgentWritabilityChanged(invokeLater);
                    }
                    break;
                }
            }
        ...
        void FireAgentWritabilityChanged(bool invokeLater)
        {
            IAgentPipeline pipeline = this.Agent.Pipeline;
            if (invokeLater)
            {
                this.Agent.EventLoop.Execute(p => ((IAgentPipeline)p).FireAgentWritabilityChanged(), pipeline);
            }
            else
            {
                pipeline.FireAgentWritabilityChanged();
            }
        }

        /// <summary>
        /// Returns the number of flushed messages in this <see cref="AgentOutboundBuffer"/>.
        /// </summary>
        public int Size => this.flushed;

        /// <summary>
        /// Returns <c>true</c> if there are flushed messages in this <see cref="AgentOutboundBuffer"/>, otherwise
        /// <c>false</c>.
        /// </summary>
        ...
        public void FailFlushed(Exception cause, bool notify)
        {
            ...
            // indirectly (usually by closing the Agent.)
            //
        ...
        sealed class CloseAgentTask : IRunnable
        {
            readonly AgentOutboundBuffer buf;
            readonly Exception cause;
            readonly bool allowAgentOpen;

            public CloseAgentTask(AgentOutboundBuffer buf, Exception cause, bool allowAgentOpen)
            {
                this.buf = buf;
                this.cause = cause;
                this.allowAgentOpen = allowAgentOpen;
            }

            public void Run() => this.buf.Close(this.cause, this.allowAgentOpen);
        }

        internal void Close(Exception cause, bool allowAgentOpen)
        {
            if (this.inFail)
            {
                this.Agent.EventLoop.Execute(new CloseAgentTask(this, cause, allowAgentOpen));
                return;
            }

            this.inFail = true;

            if (!allowAgentOpen && this.Agent.Open)
            {
                throw new InvalidOperationException("close() must be invoked after the Agent is closed.");
            }
        ...
        internal void Close(ClosedAgentException cause) => this.Close(cause, false);

        static void SafeSuccess(TaskCompletionSource promise)
        {
            // TODO:AgentPromise
            // Only log if the given promise is not of type VoidAgentPromise as trySuccess(...) is expected to return
            // false.
        ...
        {
            // TODO:AgentPromise
            // Only log if the given promise is not of type VoidAgentPromise as tryFailure(...) is expected to return
            // false.
        ...
        {
            long bytes = this.Agent.Configuration.WriteBufferHighWaterMark - this.totalPendingSize;
        ...
        {
            long bytes = this.totalPendingSize - this.Agent.Configuration.WriteBufferLowWaterMark;
        ...
        /// <see cref="AgentOutboundBuffer"/> until <see cref="IMessageProcessor.ProcessMessage"/> returns
